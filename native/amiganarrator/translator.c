...
// Off by default - see the matching comment in narrator.c.
static int _trace = 0;
...
void instr_hook_callback(unsigned int pc)
{
    ...
    // m68k_disassemble() (the expensive part - full instruction
    // decode+format) is otherwise only needed above for the trace print;
    // the STOP check below reads the opcode directly and doesn't need it.
