...
// Off by default: instr_hook_callback runs on every emulated 68k
// instruction, and m68k_disassemble()+fprintf() there dominates wall time
// (~1.8s/request vs <0.1s with tracing off) even when stderr is
// redirected to /dev/null, since the formatting work still happens.
// Pass -v to restore the original always-on trace for debugging.
static int _trace = 0;
...
void instr_hook_callback(unsigned int pc)
{
    ...
    if (_trace) {
        ...
        // m68k_disassemble() is the expensive part (full instruction
        // decode+format), called here on every single emulated
        // instruction just so the JSR trap below can learn instr_size.
        // 0x4eae is a single exact opcode ("jsr d16(A6)"), not a masked
        // group - it's always the fixed 2-word/4-byte form, so that's
        // knowable without decoding at all.
        instr_size = 4;
    }
